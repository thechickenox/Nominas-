...
import { setRequestPermits, getAllEmployees } from '@/services/FirestoreFunctions'
//Importacion de datos
...
const permitTypes = ['Vacaciones', 'Permiso', 'Enfermedad', 'Urgente']
async function submit() {
  ...
  try {
    let formValueData = form.value
    let response = await setRequestPermits(formValueData)
    router.push('/permits')
    Swal.fire({
      title: 'Exito!',
...
function checkForm() {
  ...
  if (!typeValue || typeValue.trim() === '') {
    console.log('Por favor, seleccione un tipo válido.')
    return false
  }
    ...
    <div class="form-container">
      <form @submit.prevent="submit">
        ...
        <div class="employee-data">
          ...
          <div class="input-container">
            <div class="title">Tipo de Permiso</div>
            <v-select
              clearable
              v-model="type"
              :items="permitTypes"
